...
<tr valign="top">
    ...
    <td>
        <span class="setting-description"><?php _e('Ad quality tools help small and medium sized publishers (like many Wordpress bloggers) get access to larger advertisers with bigger budgets.  These tools can be installed automatically.', 'advman'); ?></span><br><br>
        <fieldset>
            <legend class="hidden"><?php _e('Ad Quality', 'advman'); ?></legend>
            <label for="advman-enable-adjs"><input name="advman-enable-adjs" type="checkbox" id="advman-enable-adjs" value="1"<?php echo $oxEnableAdjs ? ' checked="checked"' : ''; ?> /> <?php _e('Allow URL Authentication (Experimental)', 'advman'); ?></label>
        </fieldset><br>
        <span class="setting-description"><?php _e("By turning on Url Authentication, your blog which will use javascript from Ad.js to verify the URL of your blog in the user's web browser.  Installation is automatic.  An email will be sent to the admin from time to time with an ad quality report.<br><br><strong>Please Note:  Your blog domain and admin email address will be sent to Ad.js to obtain a client ID.</strong>", 'advman'); ?></span><br><br>
    </td>
</tr>
<tr valign="top">
	<th scope="row">
