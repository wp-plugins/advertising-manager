...
class Advman_List
{
    ...
    function init()
    {
        global $advman_engine, $advman_list_table;
        $advman_list_table = new Advman_Template_Table();
        ...
        if ($action) {
            ...
            if( 'copy' === $action ) {
                if (!empty($_REQUEST['ad'])) {
                    ...
                    if (is_array($_REQUEST['ad'])) {
                        ...
                        $id = intval($_REQUEST['ad']);
                        if ($id) {
                            check_admin_referer( "copy_ad_$id" );
                            $advman_engine->copyAd($id);
                        }
                    }
            ...
            $url = remove_query_arg(array('action', 'ad', '_wpnonce'));
            wp_redirect($url);
        }
    }
    ...
    function process()
    {
        global $advman_list_table;
        echo '<div class="wrap"><h2>' . __('Ads', 'advman');
        if ( ! empty( $_REQUEST['s'] ) )
            printf( ' <span class="subtitle">' . __('Search results for &#8220;%s&#8221;', 'advman') . '</span>', get_search_query() );

        echo '</h2>';
        $advman_list_table->views();
        $advman_list_table->prepare_items();
        ?>
        <form method="get">
            <input type="hidden" name="page" value="advman-list">
            <?php
            $advman_list_table->search_box( 'search', 'ad' );
            ?>
        </form>
        <form method="post">
            <?php
            wp_nonce_field('advman-bulk-actions');
            $advman_list_table->display();
            ?>
        </form>
        </div>
    <?php
    ...
    function add_css() {
        ...
        echo '.wp-list-table .column-stats { width: 10%; }';
