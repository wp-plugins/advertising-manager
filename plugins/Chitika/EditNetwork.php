...
class Template_EditNetwork_Chitika extends Template_EditNetwork
{
	function Template_EditNetwork_Chitika()
	{
		// Call parent first!
...
<?php
	...
	{
		$fontTitle = $ad->get_network_property('font-title');
		$fontText = $ad->get_network_property('font-text');
...
?>	<table id="advman-settings-colors" width="100%">
	<tr>
		...
		<td>
			<div id="ad-color-bg" style="margin-top:1em;width:200px;background: #<?php echo ($ad->get_network_property('color-bg')) ? $ad->get_network_property('color-bg') : 'FFFFFF'; ?>;">
				<div id="ad-color-border" style="font: 10px arial, sans-serif; border: 1px solid #<?php echo ($ad->get_network_property('color-border')) ? $ad->get_network_property('color-border') : 'FF0000'; ?>" class="linkunit-wrapper">
					<div id="ad-color-title" style="color: #<?php echo ($ad->get_network_property('color-title')) ? $ad->get_network_property('color-title') : '00FFFF'; ?>; font: 11px verdana, arial, sans-serif; padding: 2px;"><b><u><?php _e('Linked Title', 'advman'); ?></u></b><br /></div>
					<div id="ad-color-text" style="color: #<?php echo ($ad->get_network_property('color-text')) ? $ad->get_network_property('color-text') : '000000'; ?>; padding: 2px;" class="text"><?php _e('Advertiser\'s ad text here', 'advman'); ?><br /></div>
					<div id="ad-color-link" style="color: #<?php echo ($ad->get_network_property('color-link')) ? $ad->get_network_property('color-link') : '008000'; ?>; font: 10px verdana, arial, sans-serif; padding: 2px;"><?php _e('www.advertiser-url.com', 'advman'); ?><br /></div>
...
<tr>
	...
	<td><input type="text" name="advman-channel" style="width:200px" id="advman-channel" value="<?php echo $ad->get_network_property('channel'); ?>" /></td>
</tr>
<tr>
	<td><label for="advman-slot"><?php _e('Alternate URL:'); ?></label></td>
	<td><input type="text" name="advman-alt-url" style="width:400px" id="advman-alt-url" value="<?php echo $ad->get_network_property('alt-url'); ?>" /></td>
</tr>
</table>
</div>
<br />
<span style="font-size:x-small; color:gray;"><?php _e('Enter a channel if you want to break out your reporting by different sections (e.g. home page, post detail)', 'advman'); ?> <?php printf(__('The Alternate URL will be called if %s does not have an ad to display.', 'advman'), $ad->network_name); ?></span>
<?php
	}
}
?>